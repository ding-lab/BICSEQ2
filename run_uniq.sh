...
do
		...
		## get the BAM file
		bamPath=$(cat ${bamMapPath} | grep WGS | grep ${Case} | grep ${SampType} | awk '{print $6}')
                while read chr; do
			if [ -s ${outputPath}${Case}_${SampType}_chr${chr}.seq ]
			then
				echo ""  >> ${outputPath}commands.txt
			else
				cm="samtools view -h ${bamPath} ${chr} | perl ${samtoolsPath} unique - | cut -f 4 > ${outputPath}${Case}_${SampType}_chr${chr}.seq"
...
cat ${outputPath}"commands.txt" | uniq | parallel -j 15 {} &
