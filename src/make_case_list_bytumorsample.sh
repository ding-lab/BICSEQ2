...
if [ $HEADER == 1 ]; then
    printf "# case\tsample_name_A\tdata_path_A\tUUID_A\tsample_name_B\tdata_path_B\tUUID_B\n"
fi
...
do
    ...
    LINE_A=$(awk -v s=$SAMPLE -v ref=$REF -v es=$ES -v st=$STA 'BEGIN{FS="\t";OFS="\t"}{if ($1 == s && $4 == es && $5 == st && $9 == ref) print}' $BAMMAP)
...
#    printf "# case\tsample_name_A\tdata_path_A\tUUID_A\tsample_name_B\tdata_path_B\tUUID_B\n"
    printf "$CASE\t$SN_A\t$PATH_C_A\t$UUID_A\t$SN_B\t$PATH_C_B\t$UUID_B\n"
