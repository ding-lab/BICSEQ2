...
function get_launch_cmd {
    SAMPLE=$1
    CASE=$(echo $SAMPLE | awk -F '.' '{print $1}')

    NMATCH=$(grep -c $SAMPLE $CASELIST )
    if [ $NMATCH -ne "1" ]; then
        >&2 echo ERROR: SAMPLE $SAMPLE matches $NMATCH lines in $CASELISTSR_H \(expecting unique match\)
        exit 1;
    fi

    CASEDATA=$(grep $SAMPLE $CASELIST)
    ...
    ARGS_SAMPLE="-o $OUTD_PROJECT_BASE/$SAMPLE $DRYARG_WORKFLOW -s $STEP"
...
#    CMD_HOST="bash /BICSEQ2/src/execute_workflow.sh $ARGS_CASE $PROJECT_CONFIG_C $CASE $SN_A $PATH_A $SN_B $PATH_B"
    CMD_HOST="bash /data5/src/execute_workflow.sh $ARGS_SAMPLE $PROJECT_CONFIG_C $CASE $SN_A $PATH_A $SN_B $PATH_B"

    # Define log path for this case, add dry run policy 
    RUN_ARGS_CASE="$RUN_ARGS $DRYARG_DOCKER -L $LOGD_PROJECT_BASE/$SAMPLE"

    CMD="bash $SCRIPT_PATH/start_docker.sh $RUN_ARGS_SAMPLE -c \"$CMD_HOST\" $DATAMAP "
...
for SAMPLE in $SAMPLES; do
    >&2 echo Processing tumor sample$SAMPLE
    ...
    if [ "$STEP" == "reset-host" ]; then
        CMD="rm -rf $LOGD_PROJECT_BASE/$SAMPLE/*"
    else
        CMD=$(get_launch_cmd $SAMPLE)
        test_exit_status

        LOGD="$LOGD_PROJECT_BASE/$AMPLE/log"
        mkdir -p $LOGD
        test_exit_status
        TMPD="$LOGD_PROJECT_BASE/$SAMPLE/tmp"
        mkdir -p $TMPD
        test_exit_status

        if [ $PARALLEL_MODE ]; then
            JOBLOG="$LOGD/process_sample.${SAMPLE}.log"
