...
# Usage: 
# bash evaluate_status.sh -S SAMPLE_LIST -p PROJECT_CONFIG -L LOGD_BASE_PROJECT [options]
...
# Required options:
# -S SAMPLE_LIST: path to SAMPLE LIST data file
# -p PROJECT_CONFIG: project configuration file.  Will be mapped to /project_config.sh in container
# -L LOGD_BASE_PROJECT: Log base dir relative to host.  Logs of parallel / bsub will be LOGD_PROJECT_BASE/SAMPLE

# Options:
# -f status: output only lines matching status, e.g., -f import:complete
# -u: include only SAMPLE in output
...
if [ ! -e $SAMPLE_LIST ]; then
    >&2 echo $SCRIPT: ERROR. CaseList does not exist: $SAMPLE_LIST
    exit 1
fi
...
function get_log {
    SAMPLE=$1
    LOGD="$LOGD_BASE_PROJECT/$SAMPLE/log"
...
# Usage: test_import_success SAMPLE LOG_FN
...
function get_job_status {
    ...
    LOG_FN=$(get_log $SAMPLE) 

    if [ $VERBOSE ]; then
        >&2 echo Case: $SAMPLE Log file: $LOG_FN
    fi
...
while read L; do
    ...
    # CaseList
    #   SAMPLE    - unique name of this tumor/normal sample
    ...
    SAMPLE=$(echo "$L" | cut -f 2) # unique ID of file

    STATUS=$(get_job_status $SAMPLE )
