...
STB="blood_normal"
HEADER=1
FF="TSV" ## file format of the output

while getopts ":db:D:r:e:A:B:O:m:H" opt; do
  case $opt in
    d)  
      DRYRUN=1
      ;;
...
do
    ...
    >&2 echo $SCRIPT: processing $SAMPLE
    ...
    LINE_A=$(awk -v sn=$SAMPLE -v ref=$REF -v es=$ES -v st=$STA 'BEGIN{FS="\t";OFS="\t"}{if ($1 == sn && $4 == es && $5 == st && $9 == ref) print}' $BAMMAP)

    if [ -z "$LINE_A" ]; then
	#>&2 echo $LINE_A
        #>&2 echo $CASES
        #>&2 echo $CASE
        >&2 echo ERROR: $REF $SAMPLE $ES $STA sample not found in $BAMMAP
        #exit 1
	continue
    elif [ $(echo "$LINE_A" | wc -l) != "1" ]; then
        >&2 echo WARNING: $REF $SAMPLE $ES $STA sample has multiple matches in $BAMMAP
