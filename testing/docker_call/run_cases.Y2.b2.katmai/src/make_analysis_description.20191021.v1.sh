...
do
    ...
    LINE_B=$(awk -v c=$CASE -v ref=$REF -v es=$ES -v st=$STB -v ds=$DISEASE 'BEGIN{FS="\t";OFS="\t"}{if ($2 == c && $3 == ds && $4 == es && $5 == st && $9 == ref) print}' $BAMMAP)
    if [ -z "$LINE_B" ]; then
    ...
    printf "$CASE\t$DISEASE\t$PATH_OUT\t$FORMAT_OUT\t$SN_A\t$UUID_A\t$SN_B\t$UUID_B\n"
